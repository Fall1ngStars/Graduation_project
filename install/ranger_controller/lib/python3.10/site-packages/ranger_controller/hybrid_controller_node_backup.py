...
class HybridChargingController(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                # 控制模式切换距离
                ('switch_distance', 1.0),         # 切换距离：距离>1.0m用Aruco，≤1.0m用点云
                
                # 控制目标
                ('aruco_goal_distance', 0.8),     # Aruco控制的目标距离
                ('pointcloud_goal_distance', 0.4), # 点云控制的目标距离
                
                # 停止参数
                ('stop_distance', 0.62),          # 停止距离
                ('stop_tolerance', 0.02),         # 停止容差
                
                # Aruco对接停止参数
                ('aruco_docking_min', 0.35),      # Aruco对接最小距离
                ('aruco_docking_max', 0.40),      # Aruco对接最大距离
                ('aruco_docking_tolerance', 0.02), # Aruco对接容差
                ...
                ('min_valid_aruco_distance', 0.2),  # Aruco最小有效距离（降低到0.2m以允许对接距离）
                ('max_valid_aruco_distance', 5.0),  # Aruco最大有效距离
                ('min_valid_pc_distance', 0.3),   # 点云最小有效距离
                ('max_valid_pc_distance', 2.0),   # 点云最大有效距离
        ...
        # Aruco对接参数
        self.aruco_docking_min = self.get_parameter('aruco_docking_min').value
        self.aruco_docking_max = self.get_parameter('aruco_docking_max').value
        self.aruco_docking_tolerance = self.get_parameter('aruco_docking_tolerance').value
        ...
        self.get_logger().info('最终混合充电控制器已启动')
        ...
        self.get_logger().info(f'停止距离: {self.stop_distance}m ± {self.stop_tolerance}m')
        self.get_logger().info(f'Aruco对接范围: {self.aruco_docking_min}m-{self.aruco_docking_max}m')
    ...
    def aruco_pose_callback(self, msg):
        ...
        if distance < self.min_valid_aruco_distance or distance > self.max_valid_aruco_distance:
            if self.enable_debug and distance > 0 and not self.docked:
                self.get_logger().debug(f'Aruco距离{self.min_valid_aruco_distance}-{self.max_valid_aruco_distance}m范围外: {distance:.2f}m，忽略')
            return
    ...
    def pointcloud_pose_callback(self, msg):
        ...
        if distance < self.min_valid_pc_distance or distance > self.max_valid_pc_distance:
            if self.enable_debug and distance > 0 and not self.docked:
                self.get_logger().debug(f'点云距离{self.min_valid_pc_distance}-{self.max_valid_pc_distance}m范围外: {distance:.2f}m，忽略')
            return
    ...
    def timer_callback(self):
        ...
        # 检查Aruco对接条件
        if self.check_aruco_docking_condition():
            self.dock_robot()
            return
    ...
    def check_aruco_docking_condition(self):
        """检查Aruco对接条件"""
        if self.aruco_distance < float('inf'):
            distance = self.aruco_distance
            
            # 检查距离是否在对接范围内
            if (self.aruco_docking_min - self.aruco_docking_tolerance <= distance <= 
                self.aruco_docking_max + self.aruco_docking_tolerance):
                if self.enable_debug and not self.docked:
                    self.get_logger().info(f'Aruco距离在对接范围内: {distance:.3f}m ∈ [{self.aruco_docking_min}m, {self.aruco_docking_max}m]')
                return True
    ...
    def dock_robot(self):
        ...
        if not self.docked and not self.emergency_stop:
            ...
            self.get_logger().info(f'🟡 对接完成，距离: {self.aruco_distance:.2f}m')
            self.publish_status()
    ...
    def determine_control_mode(self):
        ...
        elif self.aruco_distance < float('inf'):
            distance = self.aruco_distance
        else:
            distance = float('inf')
    ...
    def stop_robot(self):
        ...
        if not self.stopped and not self.emergency_stop and not self.docked:
            ...
            self.get_logger().info(f'🟢 机器人已停止，距离: {min(self.pointcloud_distance, self.aruco_distance):.2f}m')
            self.publish_status()
    ...
    def compute_aruco_control(self):
        ...
        # 在接近对接范围时减速
        if distance < self.aruco_docking_max + 0.1:  # 离对接点0.1m时开始减速
            speed_factor = max(0.1, (distance - self.aruco_docking_max) / 0.1)
            linear_x *= speed_factor
            angular_z *= speed_factor
    ...
    def apply_safety_limits(self, linear_x, angular_z):
        ...
        if self.control_mode == "ARUCO" and self.aruco_distance < float('inf'):
            if self.aruco_distance < self.aruco_docking_max + 0.2:
                linear_x *= 0.5
                angular_z *= 0.5
    ...
    def publish_status(self):
        ...
        if self.emergency_stop:
            ...
            mode_msg.data = f"DOCKED (距离:{self.aruco_distance:.2f}m)"
        elif self.stopped:
            mode_msg.data = f"STOPPED (距离:{min(self.pointcloud_distance, self.aruco_distance):.2f}m)"
        else:
        ...
        if self.enable_debug:
            ...
            if self.emergency_stop:
                ...
                self.get_logger().info(
                    f'状态: 已对接, '
                    f'Aruco距离={self.aruco_distance:.2f}m, '
                    f'对接范围={self.aruco_docking_min}m-{self.aruco_docking_max}m',
                    throttle_duration_sec=1.0
                )
            elif self.stopped:
                self.get_logger().info(
